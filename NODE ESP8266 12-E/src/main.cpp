...
const char* ssid = "ESP8266_12E_config";
...
void registerWithMaster() {
  if (registrationComplete || type.length() == 0) return;
  if (millis() - lastRegisterAttempt < REGISTER_RETRY_MS) return;

  lastRegisterAttempt = millis();
  sendRegistrationBroadcast();
  Serial.println("Broadcast dang ky gui toi ESP32...");
}
