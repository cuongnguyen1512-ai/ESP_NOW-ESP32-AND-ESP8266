...
void onDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len){
       Serial.printf("Recv len=%d, byte0=0x%02X, sizeof(SensorPacket)=%d\n", 
                len, incomingData[0], sizeof(SensorPacket));
  if(len == sizeof(SensorPacket)  && incomingData[0] == SENSOR_PACKET_MAGIC){
    SensorPacket dev;
